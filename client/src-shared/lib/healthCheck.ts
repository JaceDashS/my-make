...
async function requestHealth(url: string): Promise<RequestResult> {
  ...
  // 'web' (Electron) uses fetch — same as non-windows non-android
...
export function buildCustomHealthCheckUrl(baseUrl: string) {
  return `${normalizeBaseUrl(baseUrl)}${RUNTIME_CONFIG.CLIENT_HEALTH_PATH}`;
}
...
export async function runCustomHealthCheck(
  baseUrl: string,
  label = 'Custom',
): Promise<HealthCheckResult> {
  const url = buildCustomHealthCheckUrl(baseUrl);
  const candidates = [url];
  let lastError = 'Unknown error';

  console.log(
    `[health-check] start target=custom label=${label} platform=${getPlatformOS()} candidates=${candidates.join(
      ' | ',
    )}`,
  );
  ...
  try {
    ...
    if (response.status < 200 || response.status >= 300) {
      ...
      console.warn(
        `[health-check] non-ok response target=custom url=${url} status=${response.status} statusText=${response.statusText}`,
      );
    ...
    console.log(
      `[health-check] success target=custom url=${url} status=${response.status}`,
    );
    ...
    console.warn(
      `[health-check] request failed target=custom url=${url} error=${lastError}`,
    );
  }

  console.error(
    `[health-check] failed target=custom lastError=${lastError}`,
  );
...
export async function runHealthCheck(
  ...
  console.log(
    `[health-check] start target=${target} label=${label} platform=${getPlatformOS()} candidates=${candidates.join(
      ' | ',
    )}`,
  );
