...
function buildLocalUrls(path: string, strategy: LocalUrlStrategy) {
  ...
  if (Platform.OS === 'windows') {
    return unique(strategy.windowsHosts.map(toUrl));
  }

  if (Platform.OS === 'android') {
    return unique(strategy.androidHosts.map(toUrl));
  }

  return unique(strategy.defaultHosts.map(toUrl));
}
...
export const FALLBACK_API_URL_STRATEGY: LocalUrlStrategy = {
  ...
  defaultHosts: [RUNTIME_CONFIG.DEV_HOST_IP, LOCALHOST_HOST, LOOPBACK_HOST],
  windowsHosts: [LOCALHOST_HOST, LOOPBACK_HOST, RUNTIME_CONFIG.DEV_HOST_IP],
};
